...
def process_image_row(row, abs_path, required_chars=4, col_threshold=50, padding=5):
    ...
    caption = f"A piece of traditional Chinese calligraphy artwork from ancient times, \
author: {author}, font: {chirography}, column: {selected_index}, \
chars: {start_idx}-{start_idx+required_chars}, total columns: {len(valid_columns)}, \
total rows: {len(selected_col)}, background color: {bg_color}."
...
class CustomImageDataset(Dataset):
    def __init__(self, img_dir, img_size=512, caption_type='json', random_ratio=False, 
        ...
        caption_file = os.path.join(img_dir, 'data.csv')
        
        self.image_path = os.path.join(img_dir, 'images2')
        self.samples = pd.read_csv(caption_file)
        ...
        error_indices_path=f'error_indices_chars{required_chars}.json'
        with open(error_indices_path, 'r') as f:
            self.bad_indices = list(json.load(f))
...
if __name__ == '__main__':
    ...
    def get_item(dataset, index):
        ...
        image.save('debug_img1.png'); condition_img.save('debug_cond1.png')
        print(caption)
        
    dataset = CustomImageDataset(
        '/data/user/user64/code/word-flux/word_dataset/finalpage',
        img_size=256,
        required_chars=5
        )
